...
with open(Path("{{ cookiecutter.interpreter.dependencies.path }}").joinpath("requirements.txt"), encoding="utf-8") as f:
    requirements = [line.strip() for line in f if len(line) > 1 and "#" not in line[0:3]]

# read requirements for core development installation
extra = {}
files = [*Path("{{ cookiecutter.interpreter.dependencies.path }}").glob("**/requirements_*")]
etypes = [f.stem.split("_")[-1] for f in files]
for etype, filepath in zip(etypes, files):
    with open(filepath, encoding="utf-8") as f:
        # ignore those with commments at beginning of line
        extra[etype] = [line.strip() for line in f if len(line) > 1 and "#" not in line[0:3]]
...
setup(
    ...
    packages=find_namespace_packages(include=["{{ cookiecutter.package.name }}", "{{ cookiecutter.package.name }}.*"], exclude=['docs', 'tests']),
    ...
    package_data={"{{ cookiecutter.package.name }}": ["*.txt", "*.rst", "*.md", "*.json", "*.yaml"]},
    zip_safe=False,
)
